...
nbins = naninterpmatrix(shifttolocal(numbins,timeoffset));
nbins = [nbins(end,:); nbins; nbins(1,:)];
nbins = [nbins(:,1) nbins nbins(:,end)];
nbinsf = filter2(hf, nbins);
numbins = nbinsf(2:end-1,2:end-1);
fracbins = fracbins./numbins;

contourf(0:23, dataset.deepadcp.riforwarddepths(depthrange), fracbins', linspace(limits(1), limits(2), 15));
caxis(limits)
...
plot(zeros(size(depthrange)), dataset.deepadcp.riforwarddepths(depthrange), '>k', 'MarkerSize', 6, 'MarkerFaceColor', 'k');
hold off
grid on
ylim(ylimits);
% caxis([-2e-4 2
