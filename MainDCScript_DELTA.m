...
% cmodDC = complex_demodulate_tri(dataset.density.fulldepth.tempsraw(:,1), 1/(24*6), trilong);

cmodDC.semimaj = interp1(dataset.descriptors.datesend, deltaDaily(sst,dataset.descriptors.datesen)/2, dataset.descriptors.datesen);

disp(['Std Error on Monthly SSTA=0.28:   ', num2str(bootstrapErrors(noisebase, 1000, 0.28,30))]);
...
%Display instrumental errors
InstrumentalErrorCalcs;
